...
$this->append('tab-heading');
	...
	echo $this->Croogo->adminTab(__d('croogo', 'Photo'), '#node-photo');
...
$this->append('tab-content');
	...
	echo $this->Html->tabStart('node-photo') .
		 $this->Form->input('filename', array(
				'type' => 'file', 
				'required' => false, 
				'label'=>__d('croogo','Photo',true), 
				'onchange'=>'readURL(this);'
			)
		).
		$this->Form->input('dir', array(
				'type' => 'hidden'
			)
		).
		$this->Form->input('mimetype', array(
				'type' => 'hidden'
			)
		).
		$this->Form->input('filesize', array(
				'type' => 'hidden'
			)
		);
		...
		if ($this->request->params['action'] == 'admin_edit')
		{
			...
			{
				$preview_img = PHOTOS_PATH . '/thumb/400x400/' . $post_img;
			}
