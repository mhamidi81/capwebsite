 ...
 * @access protected
 */
	protected function _captcha($captchaProtection) {
		$continue = true;
		if (!empty($this->request->data) &&
			$captchaProtection &&
 ...
 */
	public function add() {
		...
		if (!empty($this->request->data)) {
			...
			$valid_captha = $this->_captcha(Configure::read('Service.register_recaptcha'));
			
			if ($valid_captha && $this->User->save($this->request->data)) {
	...
	public function forgot() {
		...
		if (!empty($this->request->data) && isset($this->request->data['User']['email'])) {
			...
			if (!isset($user['User']['id'])) {
				$this->Session->setFlash(__d('croogo', 'Invalid email.'), 'flash', array('class' => 'error'));
				return $this->redirect(array('action' => 'login'));
			}
 ...
 */
	...
	protected function _getSenderEmail() {
		return Configure::read('Address.email'); //'croogo@' . preg_replace('#^www\.#', '', strtolower($_SERVER['SERVER_NAME']));
	}
