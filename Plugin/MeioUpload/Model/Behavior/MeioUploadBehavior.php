 ...
 */
	...
	var $defaultOptions = array(
		...
		// v2
		'dir' => 'uploads{DS}{ModelName}{DS}images',

		// CDN version production
		//'dir' => '/home/buuutnet/public_html{DS}cdn{DS}webroot{DS}images{DS}{ModelName}{DS}images',

		// CDN version en local
		//'dir' => '/Volumes/Data/htdoc/buuut{DS}cdn{DS}webroot{DS}images{DS}{ModelName}{DS}images',
	...
	function _uploadFile(&$model, $data = null) {
		...
		foreach ($this->__fields[$model->alias] as $fieldName => $options) {
			...
			if ($options['useTable'] == false) {
				...
				$data[$model->alias][$options['fields']['mime_type']] = $data[$model->alias][$fieldName]['type'];
 ...
 */
	function _createFolders($dir, $thumbDir, $thumbsizes) {
		if ($dir[0] !== '/') 
		{
			...
			// version pour utiliser le CDN en local ou comme sous domaine
			//$dir = ROOT . '/' . $dir;
		}
		$folder = new Folder();
...
/*	function _copyFileFromTemp($tmpName, $saveAs) {
	...
	function _copyFileFromTemp($tmpName, $saveAs, $filePermission="775") {
	        $results = true;
	        if (!is_uploaded_file($tmpName)) {
	            return false;
	        }
 ...
 */
	function _unsetDataFields($modelName, $fieldName, $data, $options) {
		...
		unset($data[$modelName][$options['fields']['mime_type']]);
		return $data;
	}
